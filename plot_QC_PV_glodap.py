...
glodap = pd.read_csv('TEXT_FILES/glodap_samw_prop_all_depths.csv')
...
with_qc_rating=False 
mld_pick=True

if mld_pick: 
    profs = pd.read_csv('TEXT_FILES/glodap_samw_prop_ml_200m_mld_pick_corr.csv')
else:
    profs = pd.read_csv('/Users/dkoenig/Documents/float-basics/TEXT_FILES/glodap_samw_prop_ml_200m_MLD_03_interp_corrfile_mld.csv')

subset = profs.sort_values(by=['Float'])

# subset = subset[subset.qc_rating<5]
...
for i,row in subset.iterrows():
    ...
    dlab = 'Sigma theta' if density=='Sigma_theta' else 'Gamma n'
...
# Profile plot data (incl. PV calculation for ARGO floats)
    ...
    glodap_prof = glodap.loc[glodap.Profile == row.Profile].reset_index()
    glodap_prof['Pressure'] = gsw.p_from_z(-glodap_prof['Depth'].values,glodap_prof['Latitude'].values)
    for var in ['Spiciness','Sigma_theta','Theta','Salinity','Pressure','Oxygen','Nitrate','DIC']:
        prof_data[var] = glodap_prof[var].where(glodap_prof['Pressure'] < 800) # will be nan if csv column is empty
        if any(~np.isnan(prof_data[var])):
            prof_vars.append(var)

    glodap_prof = glodap_prof.sort_values(by='Pressure')
    try:
        pv = calcs.pv_1D(   glodap_prof['Salinity'].values, 
                            glodap_prof['Temperature'].values, 
                            glodap_prof['Longitude'].values, 
                            glodap_prof['Latitude'].values, 
                            glodap_prof['Pressure'].values)
        mask = glodap_prof['Pressure'].values < 800
...
# Profile plots T, S, PV
    ...
    for pvar in ['Theta','Salinity','PV']:
        if pvar in prof_vars:
            ...
            pres = data.index.values if pvar=='PV' else prof_data['Pressure']
            if pvar=='PV':
                ax.plot(data.values,pres,c=pcolor,zorder=5, alpha=0.5, lw=1)
            else:
                ax.plot(data.values,pres,c=pcolor,zorder=10,marker='.')
    ...
    ax.set_ylim(800,0)
    axs[0,2].grid(axis='y',c='k',ls=':',lw=0.5)
...
# Profile plots BGC
    ...
    for pvar in ['Oxygen','Nitrate','DIC']:
        if pvar in prof_vars:
            ...
            loclab = 0.03 + x*0.1
            ax.text(.02,loclab,pvar, horizontalalignment='left',
...
# MLDs
    ...
    if with_qc_rating:
        fig.savefig(f'{figure_dir}PV_{floatstr}_{profile}_qc_{qc_rating}{mldstr}.png',dpi=300)
    else:
